...
import unittest
import time
from assist.server import db
from assist.server.models import User, BlacklistToken
from assist.tests.base import BaseTestCase
...
class TestAuthBlueprint(BaseTestCase):
    ...
    def test_registered_user_incorrect_login(self):
        """ Test for login of registered user with
        Incorrect password
        """
        with self.client:
            # user registration
            ...
            self.assertTrue(resp_register.content_type == 'application/json')
            self.assertEqual(resp_register.status_code, 201)
            # registered user login with incorrect pwd
            response = login_user(self, 'joe@gmail.com', '123457')
            data = json.loads(response.data.decode())
            self.assertTrue(data['status'] == 'fail')
    ...
    def test_invalid_user_status(self):
        """ Test for user status after token expires"""
        with self.client:
            resp_register = register_user(self, 'joe@gmail.com', '123456')
            # print(resp_register.data.decode())
            time.sleep(6)
